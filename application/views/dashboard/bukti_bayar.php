		...
		<div class="col-sm-6 col-md-9 col-lg-9 p-b-50">
			<h2><?php echo $title ?></h2>
				...
				<?php if($detail_transaksi){ ?>
					...
					//form open
					echo form_open_multipart(base_url('dashboard/bukti_bayar/'.$detail_transaksi->kode_transaksi));
					 ?>
					 <table class="table">
					 	<tbody>
					 		<tr>
					 				...
					 				<select name="id_rekening" class="form-control">
					 					<?php foreach ($rekening as $rekening) { ?>
					 						<option value="<?php echo $rekening->id_rekening ?>">
					 							<?php if($pembayaran->id_rekening==$rekening->id_rekening){echo "Selected";} ?> 
					 		...
					 		<tr>
					 			<td>Tanggal Bayar</td>
					 			<td>
					 				<input type="text" name="tanggal_bayar" class="form-control-lg" placeholder="dd-mm-yyyy" value="<?php if(isset($_POST['tanggal_bayar'])){ echo set_value('tanggal_bayar'); }elseif($pembayaran->tanggal_bayar!=""){echo $pembayaran->tanggal_bayar; }else{ echo date('d-m-y'); } ?>">
					 			</td>
					 		</tr>
					 		<tr>
					 			<td>Jumlah Pembayaran</td>
					 			<td>
					 				<input type="number" name="jumlah_bayar" class="form-control-lg" placeholder="jumlah_bayar" value="<?php if(isset($POST['jumlah_bayar'])) { echo set_value('jumlah_bayar'); }elseif($pembayaran->jumlah_bayar!=""){echo $pembayaran->jumlah_bayar; }else{echo $pembayaran->jumlah_transaksi; } ?>">
					 		...
					 		<tr>
					 			<td>Dari Bank</td>
					 			<td>
					 				<input type="text" name="nama_bank" class="form-control" value="<?php echo $pembayaran->nama_bank ?>" placeholder="Nama Bank"></input>
					 		...
					 		<tr>
					 			...
					 			<td>
					 				<input type="number" name="rekening_pembayaran" class="form-control" value="<?php echo $pembayaran->rekening_pembayaran ?>" placeholder="Nama Bank"></input>
					 		...
					 		<tr>
					 			...
					 			<td>
					 				<input type="text" name="rekening_distributor" class="form-control" value="<?php echo $pembayaran->rekening_distributor ?>" placeholder="Nama Bank"></input>
